...
if [[ $is_test == "yes" ]]; then
	...
	bq --project_id=beer-spear query --n=0 --append_table --destination_table=cs686.commits -q "select '${repo}' as repo, '${sha}' as sha, '' as author_date, '' as author_name, 'graded' as status, '${txt_report}' as report, '${txt_hint}' as hint, '${txt_score}' as score, current_timestamp() as updated_at"
fi
